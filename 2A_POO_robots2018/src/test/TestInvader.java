...
/**
 * Un mini-invader... cet objet est associé à une fenêtre graphique
 * GUISimulator, dans laquelle il peut se dessiner. De plus il hérite de
 * Simulable, donc il définit deux méthodes next() et restart() invoquées par
 * la fenêtre graphique de simulation selon les commandes entrées par
 * l'utilisateur.
 */
class Invader implements Simulable {
	 ...
	 * 
	 * @param gui
	 *            l'interface graphique associée, dans laquelle se fera le dessin
	 *            et qui enverra les messages via les méthodes héritées de
	 *            Simulable.
	 * @param color
	 *            la couleur de l'invader
	 */
